...
-- exact global occurrence closure, not only that the claimed primary rows still hold their desired
-- payloads.
--
-- The reproduced defect: after a valid execution, `util.read_dataset_alias_execution_v2_primary_closure`
-- reported `live_closure_proof = true` purely from a claimed-row count, so a canonical unit-group factor
-- changed after the run was still reported as a successful current verification (root's rollback-only
-- probe: TIME_INITIAL_CLOSURE=true, UPDATE 1, TIME_SUPPORT_DRIFT_CLOSURE=true).
--
-- This replacement keeps the same signature, the same returned key set and the same claimed-row
-- readback, and additionally re-verifies on every read:
--   * the canonical target flow property and unit group by exact identity — id, version and reference
--     kind — and the operational source support the plan
--     binds: the complete source flow-property payload, its declared source unit group (the unit group
--     the alias actually references today, never the historical provenance-only hour record) and the
--     source-alias identity digest;
--   * the exact global incoming occurrence set of the plan's changed Flows across all owners and all
--     states, compared with the frozen action/occurrence identities and owner state, with any foreign
--     or non-owner-draft live occurrence refused;
--   * the remaining source-alias Flow set: any live Flow that still carries the source alias property
--     outside the completed plan refuses.
-- The support/consumer rules are the batch executor's own authorisation and closure rules replayed
-- read-only (the same reference readers and visibility boundary), so a legitimately applied plan keeps
-- passing and the executor path is unchanged. Unrelated canonical-Time consumers are outside this plan
-- and are never compared. A shape that cannot be verified fails closed rather than raising, and the
-- diagnostics never carry a foreign row payload.
...
-- ------------------------------------------------------------------------------------------------
-- The same target-reference identity guard on the pre-write side: the batch executor's support
-- authorisation must require the locked target flow property to reference exactly the declared unit
-- group by id, exact version and reference kind. This replacement is byte-identical to the reviewed
-- 20260921160000 definition except the three added conditions below; the added conditions are the only
-- delta, so the preflight, gate and service-execute simulations (which call this executor under their
-- own locks) reject an invalid relation before admission. An absent @type is the canonical unit-group
-- default of the deployed shape and keeps passing, exactly as the Length*time profile treats it.
