...
The trigger inventory in `20260805_full_schema_cutover.sql` is asserted over the schemas this repository owns, never as a database-global `not tgisinternal` total. A global total also counts the triggers the Supabase platform installs into its own schemas, and that set follows the platform image rather than this repository, so a global pin silently changes meaning between CLI versions. Measured on 2026-09-15: local CLI 2.84.2 carries one non-internal trigger in `pgmq` (the repo-authored embedding-visibility fence) plus six platform triggers on `storage.buckets`, `storage.objects`, `cron.job`, and `pgsodium.key`, and the aggregate across `auth`/`cron`/`pgmq`/`pgsodium`/`private`/`public`/`storage`/`util` is 128; CI CLI 2.116.0 observed 129, one more than the local image. The individual extra CI trigger was not identified, so the delta is observed but not attributed — do not record a specific trigger as its cause without re-measuring both images. The discriminator is the schema, not extension ownership: none of those platform triggers is a `pg_depend` extension member, so `deptype='e'` cannot separate them. The suite pins one aggregate non-internal count across `api`, `private`, `public`, and `util`, plus the exact identity and semantics of every authored trigger in the remaining schemas except the platform-owned `storage`, `cron`, and `pgsodium`. It does not pin per-schema counts for the four application schemas, so a redistribution of the same total would pass; extending the inventory means adding an explicit assertion, not raising the aggregate. The two Result guards are pinned by exact trigger name, enabled state, non-deferred state, empty column list, and exact `tgtype` 27 (`ROW|BEFORE|DELETE|UPDATE`), so an AFTER-only or DELETE-only guard fails, and their firing functions are bound to `private.zzz_guard_process_result_lifecycle()` and `private.result_process_publications_immutable_v1()`.
