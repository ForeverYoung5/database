...
| protected derivative scheduler or cron activation | Run the two guarded derivative suites above, `20260525_embedding_backpressure_and_deferred_queue.sql`, `20260728_embedding_queue_selector_plan_governance.sql`, and the three `20260923_derivative_scheduler_{budget,benchmark,history_profile}.sql` suites after a clean rebuild; run `python3 scripts/test_configure_derivative_scheduler.py` | In a dedicated disposable stack, run the `20260923_derivative_scheduler_{concurrency,upgrade,activation}.py` regressions; qualify exact-head Preview enable/rollback/enable with the versioned operation, then a fresh native test-account request after production activation | Preserve default five visits, max 25 actual row locks/visits, five combined external transitions and at most five waiting audits. Prove actor/batch fairness, readiness behind waiters, locked-peer progress, advisory overlap, provider pause/ACK/failure paths, historical row/heap preservation, and unchanged public envelopes. Record synthetic tick counts and single-call p95 separately from hosted worker latency. Cron activation must bind full before/job/project/function/head state in REPEATABLE READ through the supported `cron.alter_job` API; concurrent admin command changes must raise 40001 without overwrite. No automatic apply retry; unknown responses require read-only reconciliation. |
