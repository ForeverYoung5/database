...
| Review Member Contact readiness, activation, or binding | `supabase db reset`; run `supabase/tests/20260924_reviewer_contact_profile_activation.sql` | exercise the matching Edge command against an exact disposable Preview after the migration is available | Prove member-only access, missing/invalid/ready status, validated initial create, self ownership, state-100 publication, idempotency, publish-only version creation, optional rebinding, and current-bound-source fencing. Generic Contact-list behavior remains unchanged. |
