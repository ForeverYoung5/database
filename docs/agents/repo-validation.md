...
| OAuth client registry, capability grants, or PostgREST pre-request authorization | `supabase db reset --no-seed`; run `supabase/tests/20260830_oauth_client_capability_foundation.sql`, `20260831_oauth_actor_command_capabilities.sql`, `20260925_foundry60_cli_alias_oauth_capability.sql`, `20260806_api_contract_closure.sql`, and `20260805_full_schema_cutover.sql`; for an environment grant repair also run its populated upgrade harness, including `bash supabase/tests/upgrade/test_issue_638_oauth_cli_bundle_grant_upgrade.sh` and `bash supabase/tests/upgrade/test_issue_677_oauth_cli_alias_grant_upgrade.sh`, and prove the real PostgREST transport with `python3 scripts/test_protected_alias_postgrest_routing.py`; restart local Supabase before transport proof so the configured `public,api,graphql_public` schema list is active | with a locally signed test JWT containing `client_id`, prove an admitted relation read and representative create/save/delete command RPCs, reject raw table DML, an ungranted RPC, and a read-only-client mutation with SQLSTATE `42501`, then prove disabling the client invalidates prior access; for the protected Time-alias v2 surface, prove a `client_id`-bearing session over real PostgREST so the prior official class is refused with `42501` / "OAuth client is not authorized for this API route", the repaired six-capability class reaches the application null-request refusal with zero writes, and the service-only executor callback stays unreachable to actors; prove the queued callback's explicit `Content-Profile: api` reaches the intended service-only routine while the omitted header still resolves `public` and fails as 404/`PGRST202`; repeat against exact Preview before persistent Dev | Supabase Auth owns OAuth client secrets and refresh tokens. Database state stores only public `client_id`, kind, enabled/revoked state, capability grants, and audit history. First-party sessions without `client_id` preserve existing `auth.uid()` behavior. Nine public relations require the read capability; raw table DML remains ACL-closed. Only the three actor-bound CRUD command RPCs use `DB-CORE-WRITE-01`; other first-party CLI commands use `CLI-RPC-01`. Both exact LifecycleModel bundle signatures must use `EDGE-BUNDLE-01`, and tests must reject any `CLI-RPC-01` fallback before the MCP client is admitted. The official Production CLI grant must be exactly `CLI-ALIAS-02`, `CLI-RPC-01`, `DB-CORE-READ-01`, `DB-CORE-WRITE-01`, `NX-CORE-02`, and `EDGE-BUNDLE-01`; provision or repair it only through `api.svc_oauth_client_configure` after an exact row lock, before-state check, and two-signature route-manifest check. The five protected Time-alias v2 routes are gated by `CLI-ALIAS-02`: the four actor-facing routes are authenticated-only, the service-only executor callback keeps no actor reach, and the admitted one-shot callback is queued with an explicit `api` content profile so it cannot consume the attempt against the default `public` profile. A one-time automatic repair may select one client by the unique exact pre-change capability class, but must no-op on zero matches, fail closed on ambiguity, and never hardcode an environment client ID. Verify the exact after-state and one append-only audit event, then use the official CLI OAuth flow to create/read/update/read/delete one disposable LifecycleModel bundle without retaining identifiers or payloads in public evidence. The drift-repair migration must make every authenticated API ACL manifest-complete without changing ACLs or creating ambiguous overloads. Unknown routes fail closed, and the service-only configuration facade atomically replaces grants. Do not grant custom database access from OAuth scopes. |
